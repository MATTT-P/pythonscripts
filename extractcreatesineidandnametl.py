...
def extract_creates(file_path):
    ...
    Rewrites CREATE TABLE -> CREATE TABLE IF NOT EXISTS
    and injects promotionid + promotionname columns.
    Only extracts tables in ALLOWED_TABLES.
    """
    current_delim = ';'
    accum = []
    recording = False
...
def table_allowed(statement: str) -> bool:
    """
    Returns True if the CREATE TABLE statement is for an allowed table.
    """
    m = re.match(r'^\s*CREATE\s+TABLE\s+(IF\s+NOT\s+EXISTS\s+)?`?(\w+)`?', statement, re.I)
    if m:
...
def rewrite_create(statement: str) -> str:
    ...
    - Inject promotionid + promotionname into table definitions.
    """
    ...
    if re.match(r'^\s*CREATE\s+TABLE\b', statement, re.I):
        ...
        last_paren = statement.rfind(')')
        if last_paren != -1:
            before = statement[:last_paren].rstrip()
            after = statement[last_paren:]
